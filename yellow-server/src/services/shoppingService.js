...
【0 - 不是敏感商品】
以下商品应判断为0（不是敏感商品）：
- 普通药品：眼药水、感冒药、止痛药、退烧药、维生素、钙片、胃药、消炎药、创可贴、医用口罩等常规药品和医疗用品
- 普通日用品：洗发水、沐浴露、牙膏、牙刷、纸巾、湿巾、洗衣液、洗洁精、垃圾袋、保鲜膜等日常清洁用品
- 普通食品：零食、饮料、水果、蔬菜、米面粮油、调料等正常食品
- 普通化妆品：面霜、乳液、精华、面膜、口红、粉底等常规护肤品和彩妆（不含催情、情趣等特殊功效）
- 普通服装：T恤、裤子、鞋子、内衣（普通款）、袜子等常规服饰
- 普通电子产品：手机、充电器、数据线、耳机等常规电子产品
- 普通家居用品：床单、被套、枕头、毛巾（普通款）、拖鞋等常规家居用品
- 其他：书籍、文具、玩具、运动用品等正常商品

【低 - 轻度敏感】
有一定关联到隐私或特殊行为，但属于个人正常需求范围：
- 验孕相关：验孕棒、早孕试纸、HCG检测试纸等
- 私密护理：私处护理液、私密清洁用品（不含催情功效）
- 滋补品（特殊功效）：滋补精华液、壮阳保健品（需谨慎判断，与普通保健品区分）
- 其他：与隐私、健康检测相关的个人用品

【中 - 中度敏感】
可能是日常用品但也可用于特殊用途，且购买数量较少（总数量<10）：
- 避孕用品：安全套、避孕套（数量<10只）
- 一次性用品（特殊场景）：一次性毛巾、一次性床单、一次性内裤等（可能用于酒店、临时场所）
- 情趣用品：包含"催情"、"激情"、"快感"、"名流"、"情趣"等关键词的商品（非食物类）
- 特殊护理：特殊功效的润滑剂、护理液等
- 其他：可能用于特殊场景的日用品

数量计算示例：
"名流 水多多避孕套5只装 玻尿酸超薄免洗男用安全套 5只/盒(包装随机)×1;超大包悬挂式洗脸巾一次性棉柔巾美容院纯棉吸水不掉毛底部抽取式擦脸巾 一次性洗脸面巾 1包×1" 
-> 5只×1 + 1包×1 = 6（小于10，判断为"中"）

【高 - 高度敏感】
与"中"相同的商品类型，但购买数量较大（总数量>=10），可能用于商业或特殊用途：
- 避孕用品：安全套、避孕套（数量>=10只）
- 一次性用品：大量购买的一次性毛巾、一次性床单等（数量>=10）
- 情趣用品：大量购买的情趣用品（数量>=10）

数量计算示例：
"名流 水多多避孕套50只装 玻尿酸超薄免洗男用安全套 50只/盒(包装随机)×1" 
-> 50只×1 = 50（大于等于10，判断为"高"）

"[名流]之夜水多多玻尿酸超薄大油量免洗避孕套100只装/装×1" 
-> 100只×1 = 100（大于等于10，判断为"高"）

"名流 MO滋养玻尿酸避孕套 10只/盒×2;名流 MO滋养玻尿酸避孕套 10只/盒×1" 
-> 10只×2 + 10只×1 = 30（大于等于10，判断为"高"）

【判断原则】
1. 优先判断是否为普通商品（0），如果明显是日常用品、普通药品、普通食品等，直接返回0
2. 如果包含敏感关键词（催情、激情、快感、名流、情趣、避孕套、安全套等），再判断数量
3. 数量<10且为敏感商品 -> "中"
4. 数量>=10且为敏感商品 -> "高"
5. 验孕棒、私密护理等个人正常需求 -> "低"
